...
from nivel import verificar_nivel_completo, subir_nivel  # NUEVO
...
while corriendo:
    ...
    if tiempo_acumulado >= tiempo_por_paso:
        ...
        # ── NUEVO: verificar si el nivel está completo ──────────────────────
        # Después de cada paso del juego revisamos si quedó algún punto.
        # Si no queda ninguno, subimos de nivel.
        if verificar_nivel_completo(mapa):
 
            # Sube el nivel, resetea el mapa y devuelve la nueva velocidad.
            tiempo_por_paso = subir_nivel(score_manager, mapa, tiempo_por_paso)
 
            # Reseteamos el modo asustado por si estaba activo cuando terminó el nivel.
            modo_asustado = False
            contador_tiempo_asustado = 0
 
            # Volvemos a calcular las posiciones iniciales del mapa recién reseteado
            # y mandamos a todos los personajes a sus lugares de inicio.
            pos_pac, pos_bli, pos_pin, pos_ink, pos_cly = mapa.obtener_posiciones_iniciales()
            ...
            # Volvemos todos los fantasmas a modo scatter al empezar el nivel.
            for f in fantasmas:
                f.cambiar_modo("scatter")
        # ── FIN NUEVO ───────────────────────────────────────────────────────
